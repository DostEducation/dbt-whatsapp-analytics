with questions as (select * from {{ ref('stg_node_configs') }}),
    ...
    cross_join_contacts_questions as(
        ...
        where true and question_english is not null
    ),
    get_output_name as (
        select
            cross_join_contacts_questions.*,
            output_name as question_type
    ...
    select_latest_record_for_flow_result as (
        select contact_phone, node_label, result_input,desired_response, max(flow_result_inserted_at) as flow_result_inserted_at 
...
where output_id in ('32', '33', '34') and result_input is not null